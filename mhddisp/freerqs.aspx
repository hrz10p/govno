<%@ Page Title="" Language="C#" MasterPageFile="~/mhddisp/mhddisp.Master" AutoEventWireup="true" CodeBehind="freerqs.aspx.cs" Inherits="BANURNS.mhddisp.freerqs" %>
          ...
          <div class="clearfix"></div>
            ...
            <div class="row">
             ...
             <%--       <p>Для загрузки шаблона укажите тип шаблона и дайте название шаблону.</p>--%>
                   ...
                   <div class="modal fade bs-example-modal-sm sdr" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog modal-sm">
                      <div class="modal-content">
                        ...
                        <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel2">Назначение</h4>
                        </div>
                        <div class="modal-body">
                        
                          <div>
                             <select id="DriverSelector"  class="form-control" runat="server"></select>
                          </div>
                         
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-default" data-dismiss="modal">Нет</button>
                          <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="javascript:AddToDriver()">Назначить</button>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            socket.onmessage = function (msg) {
                ...
                if (InData.Client != "*" && InData.Client != "Disp") { return; }
                if (InData.MessgeType == "unsInfo") {
                    window.location.href = "freerqs.aspx";
                    return;
                }
                if (InData.MessgeType == "Subscrib")
                {
                    new PNotify({ title: 'Отказ от заявки', text: "Водитель подписался на заявку", type: 'info', styling: 'bootstrap3' });
              
                    $(".nr").each(function (index) {
                        if ($(this).attr("data-r") == InData.Value)
                        {
        ...
        {
            var SendData = { RowID: $("#ContentPlaceHolder1_DriverSelector option:selected").val(), RefRequest: DataR }

            $.ajax({
                url: 'freerqs.aspx/AddToDriver',
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                dataType: "JSON",
                data: JSON.stringify(SendData),
                ...
                success: function (data) {
                    if (data.d.IsDone == true) {
                        ...
                        new PNotify({ title: 'Сохранение', text: "Указанная заявка была успешно назначена", type: 'info', styling: 'bootstrap3' });
                       
                        $(".nr").each(function () {
                            if (SendData.RefRequest == $(this).attr("data-r")) {
                                $(this).parent().parent().remove();
                            }
                        });
