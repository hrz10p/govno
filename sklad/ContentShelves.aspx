<%@ Page Title="" Language="C#" MasterPageFile="~/sklad/Sklad.Master" AutoEventWireup="true" CodeBehind="ContentShelves.aspx.cs" Inherits="BANURNS.sklad.ContentShelves" %>
     ...
     <div class="">
         ...
         <div class="row">
             ...
             <%--       <p>Для загрузки шаблона укажите тип шаблона и дайте название шаблону.</p>--%>
                        ...
                        <div style="width:100%" class="control-label col-md-3 col-sm-3 col-xs-12">
           ...
           <div class="modal fade bs-example-modal-lg md2" tabindex="-1" role="dialog" aria-hidden="true">
                    ...
                    <div id="CellInfoForms">
                      ...
                      <div class="item form-group">
                        ...
                        <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Заполнить по товарному коду<span class="required"></span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <select id="BarCodeSelector" datatextfield="BarcodeValue" datavaluefield="Barcode" runat="server" class="form-control required"></select>
                        </div>
                      </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="FotterPLaceHolder" runat="server">
    ...
    <script>
        ...
        $('#ContentPlaceHolder1_BarCodeSelector').change(function () {
            if ($('#ContentPlaceHolder1_BarCodeSelector option:selected').val() != "0"){
                var SendData = { Barcode: $('#ContentPlaceHolder1_BarCodeSelector option:selected').val()};
                $('#SourceText').attr("disabled", "disabed");
                $('#EdIzm').attr("disabled", "disabled");
                $.ajax({
                    url: 'ContentShelves.aspx/GetBarcodeData',
                    type: 'POST',
                    contentType: "application/json; charset=utf-8",
                    dataType: "JSON",
                    data: JSON.stringify(SendData),
                    ...
                    success: function (data) {
                        $('#SourceText').val(data.d.Value);
                        $('#EdIzm').val(data.d.EdIzm);
                ...
                $('#SourceText').removeAttr("disabled");
                $('#EdIzm').removeAttr("disabled");
        ...
        function AddToCellData() {
            ...
            if ($('#ContentPlaceHolder1_BarCodeSelector option:selected').val() == '0') {
                SendData = {
                    ...
                    EdIzm: $("#EdIzm").val(), Count: $("#VGCount").val(), Barcode: null
                }
            } else {
                SendData = {
                    DryWarehouseID: $("#ContentPlaceHolder1_DepotSelector option:selected").val(), CellID: ClickButton.attr("data-pos"), Name: $("#SourceText").val(),
                    EdIzm: $("#EdIzm").val(), Count: $("#VGCount").val(), Barcode: $('#ContentPlaceHolder1_BarCodeSelector option:selected').val()
                }
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.d.IsDone == true) {
                        ...
                        $(".CellDataBody").html(htmlTableContent);
                        InitDeleteCellValueClick();
                        $('#ContentPlaceHolder1_BarCodeSelector').val('0');
                        $('#SourceText').removeAttr("disabled");
                        $('#EdIzm').removeAttr("disabled");
