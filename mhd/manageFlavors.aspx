<%@ Page Title="" Language="C#" MasterPageFile="~/mhd/mhd.Master" AutoEventWireup="true" CodeBehind="manageFlavors.aspx.cs" Inherits="BANURNS.mhd.manageFlavors" %>
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
                    ...
                    <form id="frm2"   class="form-horizontal form-label-left" >
                       ...
                       <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Отображение на сайте<span class="required">*</span> </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                           
                        <select id="ShowInPageSelector" class=" form-control">
                            <option value="true" >Показывать</option>
                            <option value="false">Не показвать</option>
                        </select>
                        </div>
                      </div>
                        ...
                        <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Старая цена (kzt)<span class="required">*</span> </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input id="OldPrise" class="form-control col-md-7 col-xs-12" value="1" required="required"   min="1"  type="number"/>
                        </div>
                        </div>
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          ...
                          <input id="Size" class="form-control col-md-7 col-xs-12"    min="10"  type="number"/>
                        </div>
                      </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="FotterPLaceHolder" runat="server">
    ...
    <script>
            ...
            init: function () {
                ...
                myDropzone.on('sending', function (file, xhr, formData) {
                    ...
                    if (OldFileName != file.Name) {
                        ...
                        formData.append('ShowInPage', $("#ShowInPageSelector option:selected").val());
                        formData.append('OldPrise', $("#OldPrise").val());
                ...
                HaveError = true;
                new PNotify({ title: 'Ошибка', text: resp, type: 'error', styling: 'bootstrap3' });
            },
            success: function (file, response) {
                if (response.code == 501) { // succeeded
                    return file.previewElement.classList.add("dz-success"); // from source
                } else {
                    if (response.code == undefined) { window.location.href = "manageFlavors.aspx"; }
                 //   new PNotify({ title: 'Ошибка', text: response.code, type: 'error', styling: 'bootstrap3' });
                }
        ...
        function SaveNewTemplate() {
            ...
            if (Havefiles == false) {
                new PNotify({ title: 'Ошибка', text: "Добавьте изображение", type: 'error', styling: 'bootstrap3' });
                return;
            }
        ...
        function DeleteFlavor()
        {
            ...
            $.ajax({
                url: 'manageFlavors.aspx/DeleteFlavor',
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                dataType: "JSON",
                data: JSON.stringify(SendData),
