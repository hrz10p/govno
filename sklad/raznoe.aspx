<%@ Page Title="" Language="C#" MasterPageFile="~/sklad/Sklad.Master" AutoEventWireup="true" CodeBehind="raznoe.aspx.cs" Inherits="BANURNS.sklad.raznoe" %>
           ...
           <div class="row">
             ...
             <form id="frm2"  class="form-horizontal form-label-left" > 
                  <asp:Literal runat="server" ID="RQIDLiteral"></asp:Literal>
                   ...
                   <asp:Literal runat="server" ID="RaznoeRows"></asp:Literal>
                      </tbody>
                    </table>
                        </div>
                    ...
                    <div class="modal-dialog modal-sm">
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          ...
                          <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="javascript:Delete()">Да. Удалить!</button>
                        </div>
                   ...
                   <%--     <div>Отправитель заявки:<h3 runat="server" id="RequestSender"></h3> </div>--%>
                     ...
                     <table id="MTable" class="table">
                         <thead>
                             <tr>
                                 <th>Наименование</th>
                                   <th>Количество</th>
                                 <th>Еденица иземерения</th>
                                 <th></th>
                             </tr>
                         </thead>
                         <tbody>
                               <asp:Literal runat="server" ID="RaznoeShowData"></asp:Literal>
                         </tbody>
                     </table>
                     ...
                     <div class="form-group">
                        <div class="col-md-6 col-md-offset-3">
                          <input id="SaveCh" onclick="javascript: SaveData()" type="button" value="Сохранить значения"  class="btn btn-success" />
                             ...
                             <input id="Vidacha"  onclick="javascript: window.location.href = 'deliveryB.aspx?rid=' + $('#RQID').attr('data-rq');" type="button" value="Отправить на выдачу"  class="btn  btn-danger" />
...
<asp:Content ID="Content3" ContentPlaceHolderID="FotterPLaceHolder" runat="server">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('.WButton').remove();
            $('#TopMenu').append("   <li class=\"\"><a target=\"_blank\" href=\"CellsReport.aspx?rid=" + $('#RQID').attr('data-rq') + "\">Печать заявки</a></li>\"");
        ...
        function Delete() {
            var SendData = {
                RID: $(ClickElm).attr('data-r')
            };

            $.ajax({
                url: 'Raznoe.aspx/Remove',
                type: 'POST',
                ...
                success: function (data) {
                    if (data.d.IsDone == true) {
                        ...
                        window.location.href = "Raznoe.aspx?rid=" + $('#RQID').attr('data-rq');
        ...
        function AddComment() {
            var SendData = { CommentText: $('#CommentText').val(), RequestID: $('#RQID').attr('data-rq') };

            $.ajax({
                url: 'raznoe.aspx/AddComment',
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                dataType: "JSON",
                data: JSON.stringify(SendData),
                ...
                success: function (data) {
                    if (data.d.IsDone == true) {
                        ...
                        window.location.href = "raznoe.aspx?rid=" +SendData.RequestID;
        ...
        function SaveData() {
            ...
            var SendData = { refRequest: $('#RQID').attr('data-rq'), Data: Data }


            $.ajax({
                url: 'raznoe.aspx/SaveChanges',
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                dataType: "JSON",
                data: JSON.stringify(SendData),
                ...
                success: function (data) {
                    if (data.d.IsDone == true) {
                        ...
                        window.location.href = 'raznoe.aspx?rid=' + SendData.refRequest;
        ...
        function AddToRequest() {
            ...
            var SendData = { refRequest: $('#RQID').attr('data-rq'), DWIR: Raznoe };

            $.ajax({
                url: 'raznoe.aspx/AddToRequest',
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                dataType: "JSON",
                data: JSON.stringify(SendData),
        ...
        function CloseRequest() {
            ...
            SendtoServerDataStd("deliveryB.aspx/CloseRequest", { RequestID: $('#RQID').attr('data-rq'), Message: "close_button_click", DepID: $('#RQID').attr('data-dp') }, function (resp) {
